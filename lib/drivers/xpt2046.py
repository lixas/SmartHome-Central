...
from lib.EventObserver import Event
...
class xpt2046:
    ...
    def __init__(self, miso=-1, mosi=-1, clk=-1, cs=25, spihost=esp.HSPI_HOST, half_duplex=True, mhz=5, max_cmds=16,
                 cal_x0=3783, cal_y0=3948, cal_x1=242, cal_y1=423, transpose=False, samples=3, invert_x=False, invert_y=False):
    ...
    def spi_init(self):
        ...
        if buscfg.miso_io_num >= 0 and buscfg.mosi_io_num >= 0 and buscfg.sclk_io_num >= 0:
            ...
            ret = esp.spi_bus_initialize(self.spihost, buscfg, 0)
            if ret != 0:
    ...
    def get_med_coords(self, count: int):
        ...
        if int(x_values[0]) == 0 or int(y_values[0]) == 0:
            return None
    ...
    def get_coords(self):
        ...
        if self.invert_x:
            raw_x = self.MAX_RAW_COORD - raw_x
        if self.invert_y:
            raw_y = self.MAX_RAW_COORD - raw_y
